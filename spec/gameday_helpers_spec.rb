...
require_relative "../lib/mlb_gd2/helpers"
...
describe "Helpers" do
  ...
  describe "::build_base_url" do
    context "valid days as strings" do
      it "returns a valid GD2 url" do
        expect(build_base_url(date)).to eq "http://gd2.mlb.com/components/game/mlb/year_2014/month_06/day_08/"
      end
    end
  end
  ...
  describe "::generate_nokogiri_html" do
    it "returns a Nokogiri::HTML::Document from the passed-in url" do
      expect(generate_nokogiri_html(html_url)).to be_a(Nokogiri::HTML::Document)
    end
  end

  describe "::generate_nokogiri_xml" do
    it "returns a Nokogiri::XML::Document from the passed-in url" do
      expect(generate_nokogiri_xml(xml_url)).to be_a(Nokogiri::XML::Document)
    end
  end
end